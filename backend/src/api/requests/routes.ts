...
router.get("/view_requests/me", (req, res) => {
  ...
  const userRequests = viewRequestsSentToUser(
...
router.get("/view_requests/:groupid", (req, res) => {
  const payload = req.body;
  const groupRequests = viewRequestsForGroup(
    req.params.groupid,
    payload.requestStatus
  );
...
router.get("/view_requests", (req, res) => {
  ...
  const userRequests = viewRequestsSentByUser(
    tokenString as string,
    payload.requestStatus
  );
