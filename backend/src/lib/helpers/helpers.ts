...
export function getRequestsForGroup(
  groupId: string,
...
export function getRequestsSentByUser(
  userId: string,
  requestStatus?: RequestStatus
): Request[] {
  let data: Data = getData() as Data;
...
export function getRequestsSentToUser(
  userId: string,
  requestStatus?: RequestStatus
): Request[] {
  let data: Data = getData() as Data;
  ...
  let groupsUserIsLeaderFor = data.groups.filter(
    (group) => group.members[0].userId === userId
  );
  let requestsSentToUser = requests.filter((request) =>
    groupsUserIsLeaderFor.some((group) => group.groupId === request.groupId)
  );
  return requestsSentToUser;
}
